...
int main (int argc, char *argv[])
{
	...
	//Produce NUMPROD volte
	for(i = 0; i < NUMPROD; i++){
	
		Wait_Sem(id_sem, SPAZIO_DISP);
		//Se mi trovo nella sezione critica allora sicuramente almeno uno dei buffer 
		//è disponibile per la produzione
		if(msg1->stato == LIBERO){
			//Pone lo stato del buffer in IN_USO
			msg1->stato = IN_USO;
			//Produzione
			printf("Produttore: PRODUZIONE del valore %d sul buffer 1\n\n", i);
			msg1->valore = i;
			//Pone lo stato del buffer in OCCUPATO
			msg1->stato = OCCUPATO;
			//Risveglia l'eventuale consumatore
			Signal_Sem(id_sem, MEX_DISP);
		}else if(msg2->stato == LIBERO){
			//Se il primo non è disponibile sicuramente il secondo lo sarà
			//Pone lo stato del buffer in IN_USO
			msg2->stato = IN_USO;
			//Produzione
			printf("Produttore: PRODUZIONE del valore %d sul buffer 2\n\n", i);
			msg2->valore = i;
			//Pone lo stato del buffer in OCCUPATO
			msg2->stato = OCCUPATO;
			//Risveglia l'eventuale consumatore
		...
		//Attesa di 1 secondo per ogni produzione
		sleep(1);
